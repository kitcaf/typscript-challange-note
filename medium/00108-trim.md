...
```typescript
type C = " " | "\n" | "\t";
type TrimLeft<S extends string> = S extends `${C}${infer Re}` ? Trim<Re> : S;
type TrimRight<S extends string> = S extends `${infer Re}${C}` ? Trim<Re> : S;

type Trim<S extends string> = TrimRight<TrimLeft<S>>;
```
### 问题

一、为什么不能直接一起递归左右两边的空格？

因为目标是去掉左右两边的所有空格，对于的`${C}${infer Re}${C}`假设没有右边空格，其实就比匹配了。左边空格也无法去掉了。此时的匹配规则是**左右两边都有空格**
```typescript
type Trim<S extends string> = S extends `${C}${infer Re}${C}` ? Trim<Re> : S;
```
...
- [原题链接](https://github.com/type-challenges/type-challenges/tree/main/questions/00108-medium-trim)
