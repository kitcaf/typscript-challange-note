...
```typescript
type Last<T extends any[]> = T extends [...infer _La, infer F] ? F : never;
```
`... infer`放在元组类型的前面也是可以推断的。注意？**后面第一个才可以使用前面infer的变量**
:之后无法使用。
...
- [原题链接](https://github.com/type-challenges/type-challenges/tree/main/questions/00015-medium-last)
